...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements UserService {
    ...
    public ResponseResult<List<UserInfoVo>> getAll() {
        ...
        wrapper.eq(User::getStatus, SysContants.USER_STATUS_NORMAL);
    ...
    public ResponseResult<Object> removeById(Long id) {
        ...
        // 3. 进行删除用户
        boolean update = lambdaUpdate().eq(User::getId, id).set(User::getDelFlag, SysContants.DEL_DELETED).update();
        if(update){
